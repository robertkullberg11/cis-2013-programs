...
if (stringPassword.length < 8)       //tests to see that password is at least 8 characters
{
      stringOutput = "Your password is less than 8 characters!";
} 
      else
      {
         for (var a=0; a < arrayBadPasswords.length; a++)     //loop takes values with 8 characters or more and tests them against array
         {
            if (stringPassword === arrayBadPasswords[a])      //words that are found in the array bad passwords are not allowed to be used
            {
               stringOutput = "Uh oh.  You are going to want to change your password!";
               break;  //breaks the loop
            }
            else                                              //words not found in the array are good passwords.
            {
                stringOutput = "Great job! Your password is safe!";
            }
         }
      }
   $("output").value = stringOutput;                          //Sends to the DOM
};
